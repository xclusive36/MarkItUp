...
export class PluginBridge {
  ...
  private static detectAICapabilities(plugin: PluginManifest): AIPluginMetadata['aiIntegration'] {
    ...
    // const aiKeywords = ['ai', 'artificial intelligence', 'machine learning', 'gpt', 'claude', 'llm', 'language model']; // Commented out: not used
  ...
  static getAIPlugins(): AIPluginMetadata[] {
    return this.getAllAsAIPlugins().filter(
      plugin =>
  ...
  static getEnhanceablePlugins(): AIPluginMetadata[] {
    return this.getAllAsAIPlugins().filter(
      plugin =>
        plugin.category === 'Productivity' || plugin.category === 'Core' || plugin.contentGeneration
    );
  }
...
export function isPhase5Plugin(
  plugin: PluginManifest | AIPluginMetadata
): plugin is PluginManifest {
