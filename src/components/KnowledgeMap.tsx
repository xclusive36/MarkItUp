      ...
      notes.forEach((note, index) => {
        nodes.push({
          ...
          relevance: searchQuery ? 
            (note.content.toLowerCase().includes(searchQuery.toLowerCase()) ? 1 : 0.5) : 1,
          data: {
    ...
    if (viewOptions.showConnections) {
      mapData.edges.forEach(edge => {
        ...
        if (sourceNode && targetNode) {
          ...
          ctx.strokeStyle = edge.type === 'wikilink' ? 
    ...
    mapData.nodes.forEach((node, index) => {
      ...
      if (node.size > 8) {
        ...
        if (ctx.measureText(label).width > maxWidth) {
          label = label.substring(0, Math.floor(label.length * maxWidth / ctx.measureText(label).width)) + '...';
        }
        ...
        <div 
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1">
              ...
              <button
                onClick={() => setViewOptions(prev => ({ ...prev, showConcepts: !prev.showConcepts }))}
                className={`p-2 rounded-lg transition-colors ${
            ...
            <div className="flex items-center gap-1 border-l border-gray-300 dark:border-gray-600 pl-2">
              ...
              <button
                onClick={() => setCanvasState(prev => ({ ...prev, zoom: Math.max(0.3, prev.zoom / 1.2) }))}
                className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
                title="Zoom Out">
              ...
              <button
                onClick={() => setCanvasState({ zoom: 1, panX: 0, panY: 0, width: 800, height: 600 })}
                className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
                title="Reset View">
            ...
            <div 
              ...
              <div className="mb-4">
                <div className="flex items-center gap-2 mb-2">
                  {selectedNode.type === 'note' && <BookOpen className="w-5 h-5 text-blue-600 dark:text-blue-400" />}
                  {selectedNode.type === 'tag' && <Tag className="w-5 h-5 text-green-600 dark:text-green-400" />}
                  {selectedNode.type === 'concept' && <TrendingUp className="w-5 h-5 text-yellow-600 dark:text-yellow-400" />}
