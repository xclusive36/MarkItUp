...
export function PluginHealthMonitor({ pluginManager }: PluginHealthMonitorProps) {
  ...
  const [alerts, setAlerts] = useState<
    Array<{ pluginId: string; message: string; type: 'warning' | 'error' }>
  >([]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
        ...
        {healthData.size === 0 ? (
          <p className="text-gray-500 dark:text-gray-400 text-center py-8">No plugins loaded</p>
        ) : (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
          ...
          <p className="text-2xl font-bold text-gray-900 dark:text-white">
            {Array.from(healthData.values()).reduce(
              (sum, health) => sum + (health.responseTime || 0),
              0
        ...
        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
          ...
          <p className="text-2xl font-bold text-gray-900 dark:text-white">
            {Array.from(healthData.values()).reduce(
              (sum, health) => sum + (health.executionCount || 0),
              0
            )}
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
          ...
          <p className="text-2xl font-bold text-red-600">
            {Array.from(healthData.values()).reduce((sum, health) => sum + health.errorCount, 0)}
          </p>
        </div>
      </div>
    </div>
