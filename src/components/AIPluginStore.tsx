...
import { AIPluginMetadata, AIPluginManager, AIPluginAPI } from '@/lib/ai/plugin-manager-simple';
      ...
      <div
        ...
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center gap-3">
            <div 
              className={`p-2 rounded-lg ${categoryColor}`}>
          ...
          <div className="flex items-center gap-2">
            {plugin.aiRequirements?.requiresApiKey && (
              <div className="p-1 rounded bg-yellow-100 dark:bg-yellow-900/30" title="Requires API Key">
        ...
        <div 
          ...
          <div 
            ...
            <div className="flex items-center gap-3">
              <div className={`p-3 rounded-lg ${categoryColors[selectedPlugin.category]}`}>
                {React.createElement(categoryIcons[selectedPlugin.category] || Package, { className: "w-6 h-6" })}
              </div>
              <div>
                <h2 
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span style={{ color: theme === 'dark' ? '#9ca3af' : '#6b7280' }}>Category:</span>
                  ...
                  <div className="flex justify-between">
                    <span style={{ color: theme === 'dark' ? '#9ca3af' : '#6b7280' }}>Downloads:</span>
                    <span style={{ color: theme === 'dark' ? '#d1d5db' : '#4b5563' }}>
                      {selectedPlugin.downloads.toLocaleString()}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span style={{ color: theme === 'dark' ? '#9ca3af' : '#6b7280' }}>Pricing:</span>
                    <span style={{ color: theme === 'dark' ? '#d1d5db' : '#4b5563' }}>
              ...
              <div>
                ...
                <div className="space-y-1">
                  {Object.entries(selectedPlugin.capabilities).map(([capability, enabled]) => (
                    enabled && (
            ...
            {selectedPlugin.aiRequirements && (
              <div 
                className="p-3 rounded-lg border-l-4 border-yellow-400 bg-yellow-50 dark:bg-yellow-900/20">
                <div className="flex items-center gap-2 mb-1">
                  <AlertCircle className="w-4 h-4 text-yellow-600 dark:text-yellow-400" />
                  <span 
                    className="font-medium text-sm text-yellow-800 dark:text-yellow-200">
                    AI Requirements
                  </span>
                </div>
                <ul className="text-xs text-yellow-700 dark:text-yellow-300 space-y-1">
                  {selectedPlugin.aiRequirements.requiresApiKey && (
                    <li>• Requires AI API key</li>
                  )}
                  <li>• Supported providers: {selectedPlugin.aiRequirements.supportedProviders?.join(', ')}</li>
          ...
          <div className="flex-1 overflow-auto p-6">
            {isLoading ? (
              ...
              <>
                ...
                {activeTab === 'installed' && (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {plugins.filter(plugin => installedPlugins.has(plugin.id)).map(plugin => renderPluginCard(plugin))}
                  ...
                  <div>
                    <div className="mb-6">
                      ...
                      <p 
                        className="text-sm"
                        style={{ color: theme === 'dark' ? '#9ca3af' : '#6b7280' }}>
                        Based on your {notes.length} notes and usage patterns, we recommend these plugins:
                      </p>
                    </div>
