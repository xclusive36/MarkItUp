...
export default function BatchAnalyzer({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'detailed' | 'insights' | 'filters'>('overview');
  ...
  const [sortBy, setSortBy] = useState<'name' | 'wordCount' | 'completeness' | 'lastModified'>('name');
  ...
  const runBatchAnalysis = async () => {
    ...
    try {
      ...
      for (const note of notes) {
        ...
        const stopWords = new Set(['that', 'this', 'with', 'from', 'they', 'have', 'been', 'their', 'said', 'each', 'which', 'will', 'would', 'could', 'should', 'these', 'those', 'then', 'than', 'only', 'also', 'after', 'back', 'first', 'well', 'many', 'some', 'very', 'when', 'where', 'more', 'most', 'much', 'such']);
        ...
        const positiveWords = ['good', 'great', 'excellent', 'amazing', 'wonderful', 'fantastic', 'love', 'like', 'enjoy', 'happy', 'success', 'achieve', 'improve', 'better', 'best'];
        const negativeWords = ['bad', 'terrible', 'awful', 'hate', 'dislike', 'problem', 'issue', 'difficult', 'hard', 'fail', 'failure', 'wrong', 'error', 'mistake', 'worse', 'worst'];
        ...
        if (!note.content.includes('#')) suggestedImprovements.push('Add headers for better structure');
        if (note.tags.length === 0) suggestedImprovements.push('Add relevant tags');
        if (!note.content.includes('[[')) suggestedImprovements.push('Create connections to other notes');
        if (sentences.length < 3) suggestedImprovements.push('Expand with more detailed explanations');
        ...
        <div 
          ...
          <div className="flex items-center gap-2">
            {selectedNotes.size > 0 && (
              <span 
                className="text-sm px-3 py-1 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300">
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div 
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-sm font-medium" style={{ color: theme === 'dark' ? '#d1d5db' : '#6b7280' }}>
                          Total Notes
                        </span>
                      </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div 
                      ...
                      <h3 className="font-medium mb-4" style={{ color: theme === 'dark' ? '#f9fafb' : '#111827' }}>
                        Top Tags
                      </h3>
                      <div className="space-y-2">
                        {analyticsSummary.topTags.slice(0, 5).map((tag, index) => (
                          <div key={tag.tag} className="flex items-center justify-between">
                            <span className="text-sm" style={{ color: theme === 'dark' ? '#d1d5db' : '#6b7280' }}>
                              {tag.tag}
                            </span>
                            <div className="flex items-center gap-2">
                              <div 
                                className="h-2 bg-blue-500 rounded"
                                style={{ width: `${Math.max(20, (tag.count / analyticsSummary.topTags[0]?.count || 1) * 60)}px` }}
                              />
                          ...
                          <div key={topic.topic} className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                              <div 
                                ...
                                style={{ width: `${Math.max(20, (topic.count / analyticsSummary.topTopics[0]?.count || 1) * 60)}px` }}
                              />
                    ...
                    <table className="w-full">
                      <thead>
                        <tr 
                          className="border-b"
                          style={{ borderColor: theme === 'dark' ? '#374151' : '#e5e7eb' }}>
                          <th className="text-left p-3 text-sm font-medium" style={{ color: theme === 'dark' ? '#d1d5db' : '#6b7280' }}>
                            Select
                          </th>
                          <th className="text-left p-3 text-sm font-medium" style={{ color: theme === 'dark' ? '#d1d5db' : '#6b7280' }}>
                            Note
                      ...
                      <tbody>
                        {filteredResults.map((result) => (
                          <tr 
                            ...
                            <td className="p-3 text-sm" style={{ color: theme === 'dark' ? '#d1d5db' : '#6b7280' }}>
                              {result.wordCount}
                              <div className="text-xs text-gray-500">{result.readingTime}m read</div>
                            </td>
                            <td className="p-3">
                              <span 
